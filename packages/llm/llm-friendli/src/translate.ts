/**
 * Translate FriendliAI SSE payloads with one stateful harness block per
 ...
 *
 * FriendliAI parses reasoning into `reasoning_content` (`parse_reasoning: true`),
 * so the translator never strips `<think>` tags: the wire already separates
 * the reasoning channel from the answer channel.
 *
...
export function mapFinishReason(reason: string): FinishReason {
  switch (reason) {
    ...
    default:
      // content_filter and any future additions.
      return {
        kind: 'error',
...
/**
 * Map wire usage fields. `prompt_tokens` INCLUDES cache hits
...
export async function* translate(payloads: AsyncIterable<string>): AsyncGenerator<StreamChunk> {
  ...
  for await (const payload of payloads) {
    ...
    for (const choice of chunk.choices ?? []) {
      ...
      // Reasoning first: a reasoning model interleaves it before the answer.
